...
{   private Context context;
    ...
    {   try
        {   SharedPreferences preference= PreferenceManager.getDefaultSharedPreferences(context);
            String sort=preference.getString(SpKeys.SORT_KEY,context.getResources().getString(R.string.sort_default));
            ...
            {   JSONObject respone=new JSONObject(jsonResp.toString());
                ...
                if ( cVVector.size() > 0 ) {
                    ...
                    {   case "0":   //context.getContentResolver().delete(MoviesContract.PopularEntry.CONTENT_URI,null,null);
                                    inserted = context.getContentResolver().bulkInsert(MoviesContract.PopularEntry.CONTENT_URI, cvArray);
                                    break;

                        case "1":   //context.getContentResolver().delete(MoviesContract.RatedEntry.CONTENT_URI,null,null);
                                    inserted = context.getContentResolver().bulkInsert(MoviesContract.RatedEntry.CONTENT_URI, cvArray);
                                    break;
                    }
